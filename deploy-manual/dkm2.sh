...
#kubeFlannelId=$(juju add-machine zone=us-east-1b 2>&1 | sed 's/created machine //')
#kubeMinionId=$(juju add-machine zone=us-east-1b 2>&1 | sed 's/created machine //')
...
#sed -i "s/MACHINE_KUBE_FLANNEL_ID/"$kubeFlannelId"/g" kubernetes-bundle-patched.yaml
#sed -i "s/MACHINE_KUBE_MINION1_ID/"$kubeMinionId"/g" kubernetes-bundle-patched.yaml
